...
//          Drops users directly into Step 1 of the form.
//
...
export default function ApplyLandingPage() {
  ...
  function getStepName(s: number) {
    const names: Record<number, string> = {
      1: "loan_amount", 2: "loan_purpose", 3: "credit_score",
      4: "annual_income", 5: "employment_status", 6: "personal_info", 7: "consent",
    };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {/* Minimal header — no nav links, just trust signals */}
        ...
        <form onSubmit={handleSubmit}>
          ...
          {step === 1 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="bg-secondary/40 border border-primary/20 rounded-xl p-3 mb-6 flex items-center gap-2 text-sm text-slate-700">
                ...
                <span><strong>No impact to your credit score.</strong> We use a soft pull only.</span>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                {LOAN_AMOUNTS.map(({ label, value }) => (
                  <button
                    key={value}
                    type="button"
                    onClick={() => { update("loanAmount", value); }}
              ...
              <div className="grid grid-cols-1 gap-3">
                {LOAN_PURPOSES.map((purpose) => (
                  <button
                    key={purpose}
                    type="button"
                    onClick={() => update("loanPurpose", purpose)}
                    className={`w-full text-left px-5 py-4 rounded-xl border-2 font-medium transition-all ${
          ...
          {step === 4 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <p className="text-xs text-slate-400 mt-3">Your income information is encrypted and never shared without your consent.</p>
            </div>
          )}
          ...
          {step === 5 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <p className="text-slate-500 mb-6">Select the option that best describes your current situation.</p>
              <div className="grid grid-cols-1 gap-3">
          ...
          {step === 6 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <p className="text-slate-500 mb-6">Your personal information is encrypted and used only to match you with lenders.</p>
          ...
          {step === 7 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5 mb-6 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-slate-500">Loan amount</span><span className="font-semibold text-slate-900">${Number(form.loanAmount).toLocaleString()}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Purpose</span><span className="font-semibold text-slate-900">{form.loanPurpose}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Credit score</span><span className="font-semibold text-slate-900">{form.creditScore}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Annual income</span><span className="font-semibold text-slate-900">${Number(form.annualIncome).toLocaleString()}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Name</span><span className="font-semibold text-slate-900">{form.firstName} {form.lastName}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">State</span><span className="font-semibold text-slate-900">{form.state}</span></div>
              </div>
          ...
          <div className="mt-8 flex items-center gap-4">
            {step > 1 && (
              <button
                type="button"
                onClick={prevStep}
                className="flex items-center gap-2 text-slate-500 hover:text-slate-900 transition-colors px-4 py-3 rounded-xl hover:bg-slate-100 font-medium"
              >
            ...
            {step < TOTAL_STEPS ? (
              <button
                ...
                onClick={nextStep}
                disabled={!canAdvance()}
                className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-8 py-4 text-lg font-semibold text-white shadow-lg hover:bg-primary/90 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-primary disabled:hover:translate-y-0 hover:-translate-y-0.5"
              >
              ...
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-8 py-4 text-lg font-semibold text-white shadow-lg hover:bg-primary/90 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {isSubmitting ? "Finding your offers..." : "See My Rates"}
      ...
      <footer className="border-t border-slate-200 py-4 px-4 bg-white">
        <div className="container mx-auto max-w-2xl text-center text-xs text-slate-400 space-y-2">
          <p>
            Apruvee, LLC is a marketplace, not a lender. We do not make credit decisions or charge application fees.
            Rates shown are estimates only. Not available in all states.
          </p>
